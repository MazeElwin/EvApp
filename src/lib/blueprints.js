...
function uniqueBlueprintKey(bp, index, output, machineLabel, category) {
  const variant = [
    String(firstValue(bp.id, bp.recipeId, bp.recipe_id, bp.blueprintId, bp.blueprint_id)).trim(),
    output.typeId,
    normalizeName(output.name),
    normalizeName(machineLabel),
    normalizeName(category),
    numberOrZero(firstValue(bp.durationSeconds, bp.time, bp.buildTime, bp.manufacturingTime)),
    output.quantity,
    index
  ].join('::');
...
export function loadBlueprintData() {
  ...
  for (const bp of blueprints) {
    ...
    if (bp.output.typeId && !blueprintsByOutputTypeId.has(bp.output.typeId)) {
      blueprintsByOutputTypeId.set(bp.output.typeId, bp);
    }

    const normalizedOutputName = normalizeName(bp.output.name);
    if (normalizedOutputName && !blueprintsByOutputName.has(normalizedOutputName)) {
      blueprintsByOutputName.set(normalizedOutputName, bp);
    }
